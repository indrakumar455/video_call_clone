...
export default function VideoMeetComponent() {
  ...
  let handleVideo = () => {
    if (video) {
      // video ON → ab OFF karna hai
      const tracks = localVideoref.current?.srcObject?.getVideoTracks();
      tracks && tracks.forEach((t) => t.stop()); // -> onended fire -> blackSilence chalega
      setVideo(false);
    } else {
      // video OFF → ON karna hai
      setVideo(true);
      getUserMedia(); // dubara camera on
    }
  };
  let handleAudio = () => {
    setAudio(!audio);
                ...
                <div className="messages">
                  {messages.length !== 0 ? (
                    messages.map((item, index) => {
                      return (
                        <div key={index} className="messageData">
